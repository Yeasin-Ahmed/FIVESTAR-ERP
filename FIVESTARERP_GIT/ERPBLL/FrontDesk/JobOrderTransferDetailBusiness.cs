...
{
   public class JobOrderTransferDetailBusiness: IJobOrderTransferDetailBusiness
    {
        private readonly IFrontDeskUnitOfWork _frontDeskUnitOfWork;
        private readonly JobOrderTransferDetailRepository _jobOrderTransferDetailRepository;
        private readonly JobOrderRepository _jobOrderRepository;
        private readonly IJobOrderBusiness _jobOrderBusiness;

        public JobOrderTransferDetailBusiness(IFrontDeskUnitOfWork frontDeskUnitOfWork, IJobOrderBusiness jobOrderBusiness)
        {
            this._frontDeskUnitOfWork = frontDeskUnitOfWork;
            this._jobOrderTransferDetailRepository = new JobOrderTransferDetailRepository(this._frontDeskUnitOfWork);
...
", Utility.ParamChecker(param));
        ...
        public bool SaveJobOrderTransfer(long transferId, long[] jobOrders, long userId, long orgId, long branchId)
        {
            ...
            string transferCode= ("T-" + DateTime.Now.ToString("dd") + DateTime.Now.ToString("hh") + DateTime.Now.ToString("mm") + DateTime.Now.ToString("ss"));
            ...
            {
                IsSuccess= _jobOrderBusiness.SaveJobOrderTransfer(transferId, jobOrders,userId,orgId,branchId,string.Empty,string.Empty);
                if(IsSuccess == true){
                    GetTransferDeliveryChalan(transferCode, orgId);
                }
            }
            return IsSuccess;
        }
        ...
        public ExecutionStateWithText SaveJobOrderTransferWithReport(long transferId, long[] jobOrders, long userId, long orgId, long branchId, string cName, string cNumber)
        {
            ...
            if (_jobOrderTransferDetailRepository.Save() == true)
            {
                IsSuccess = _jobOrderBusiness.SaveJobOrderTransfer(transferId, jobOrders, userId, orgId, branchId,cName,cNumber);
        ...
        public bool UpdateReceiveJobOrder(long transferId, long jobOrderId, long userId, long orgId, long branchId)
        {
            bool IsSuccess = false;
            var transfer = GetAllTransferJob(transferId, orgId, branchId);
            if (transfer != null)
            {
                transfer.TransferStatus = JobOrderTransferStatus.Received;
                transfer.UpUserId = userId;
                transfer.UpdateDate = DateTime.Now;
                _jobOrderTransferDetailRepository.Update(transfer);
            }
            if(_jobOrderTransferDetailRepository.Save() == true)
            {
                IsSuccess= _jobOrderBusiness.UpdateTransferJob(jobOrderId, userId, orgId, branchId);
            }
            return IsSuccess;
        }

        public JobOrderTransferDetail GetAllTransferJob(long transferId, long orgId, long branchId)
        {
            return _jobOrderTransferDetailRepository.GetOneByOrg(t => t.JobOrderTransferDetailId == transferId && t.OrganizationId == orgId && t.ToBranch == branchId);
        }

        public IEnumerable<JobOrderDTO> GetTransferDeliveryChalan(string transferCode, long orgId)
        {
...
where JodOrderId IN(
Select JobOrderId From tblJobOrderTransferDetail Where TransferCode='{0}' and OrganizationId={1})
