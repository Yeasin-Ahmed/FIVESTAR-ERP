...
{
   public class ConfigurationBaseRepository<T>: IBaseRepository<T> where T : class
    {
        private readonly IConfigurationUnitOfWork _configurationUnitOfWork;
        internal DbSet<T> dbSet = null;
        public ConfigurationBaseRepository(IConfigurationUnitOfWork configurationUnitOfWork)
        {
            if (configurationUnitOfWork == null) throw new ArgumentNullException("DbContext is not assigned");
            this._configurationUnitOfWork = configurationUnitOfWork;
            dbSet = this._configurationUnitOfWork.Db.Set<T>();
        }
        ...
        public bool Save()
        {
            ...
            return _configurationUnitOfWork.Db.SaveChanges() > 0;
        }

        public async Task<bool> SaveAsync()
        {
            return await _configurationUnitOfWork.Db.SaveChangesAsync() > 0;
        }
        ...
        {
            _configurationUnitOfWork.Db.Entry(entity).State = System.Data.Entity.EntityState.Modified;
        }
        ...
        {
            using (var cmd = _configurationUnitOfWork.Db.Database.Connection.CreateCommand())
            {
                cmd.CommandText = Sql;
